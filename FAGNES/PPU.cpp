...
struct PPUCTRL {
	...
	// ASSINADO: ISIS. >:(
	uint8_t getNameTableAddr() const {
		return control & 0x03;
	}
};
...
struct PPU {
	...
	uint16_t mirrorAddress(uint16_t address) {
		...
		// Aplica espelhamento (por exemplo: vertical)
...
// Framebuffer\saída de vídeo			Alta		Precisa montar uma imagem(array 256x240) pra mostrar via SDL.
